...
// Ensure this declaration exists before use
void saveSettings();
...
void loadSettings() {
  ...
  if (EEPROM.read(0) != 0x42) {
    Serial.println("⚠️ EEPROM uninitialized — using runtime defaults.");

    // Runtime defaults (used until user hits "Set as Default")
    ...
    return;  // do not save unless explicitly told
  }
...
void saveSettings() {
  EEPROM.write(0, 0x42);  // initialized marker
